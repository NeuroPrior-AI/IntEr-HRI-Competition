...
def resnet_predict(X):
    ...
    '''
    # Load model:
    model = torch.load('Model/resnet_model/best_model.pt')
    # Load standardization parameters:
    mean = np.load('Model/resnet_model/mean.npy')
    std = np.load('Model/resnet_model/std.npy')
    # Wrap model to scikit-learn (with predict function)
    pytorch_classifier = PyTorchClassifier(model, mean, std)

    return pytorch_classifier.predict(X)
